...
for name, code in leagues.items():
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as resp:
            ...
            print(f"=== {name}: {len(events)} partidos el 2026-09-13 ===")
            for ev in events:
                ...
                if odds_list:
                    ...
                    odds_data = {
                        ...
                        'spread': od.get('spread', {}).get('details'),
                        'spread_odds': od.get('spread', {}).get('odds', {}).get('home', {}).get('close', {}).get('odds'),
                        'over_odds': od.get('underOver', {}).get('over', {}).get('close', {}).get('odds'),
                        'under_odds': od.get('underOver', {}).get('under', {}).get('close', {}).get('odds'),
                    }
                match_info = {
                    'league': name,
                    'name': name_str,
...
with open('scratch/matches_13sep.json', 'w', encoding='utf-8') as f:
    json.dump(all_matches, f, indent=2, ensure_ascii=False)

print(f"\nTotal partidos encontrados el 13-09-2026: {len(all_matches)}")